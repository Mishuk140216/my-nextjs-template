'use client';
import UserDA from "@/app/api/usersDA";
...
const Users = () => {
    ...
    useEffect(() => {
        setLoading(true);
        UserDA.get_all_users()
            .then((response) => {
                console.log(response);
                const { success, users } = response;
                if (success) {
                    const simplifiedUsers = users.map((eachUser: any) => simplifyUserObject(eachUser));
    ...
    function simplifyUserObject(user: any) {
        const eachUser = {
            id: user?._id,
    ...
    const userBody = (rowData: any) => {
        ...
        return (
            <React.Fragment>
                ...
                />
                <span style={{ marginLeft: '.5em', verticalAlign: 'middle' }}>{username}</span>
            </React.Fragment>
        );
    };
    ...
    const dateOfBirthBody = (rowData: User) => {
    ...
    const balanceBody = (rowData: User) => {
    ...
    if (loading) return (
        ...
        return (
            <div className="card pb-2">
                <h3>Users</h3>
                <DataTable value={users} paginator header={header} rows={6}
                        paginatorTemplate="FirstPageLink PrevPageLink PageLinks NextPageLink LastPageLink CurrentPageReport RowsPerPageDropdown"
                        rowsPerPageOptions={[10, 25, 50]} dataKey="id" selectionMode="checkbox" selection={selectedUsers} 
                        onSelectionChange={(e) => {
                    ...
                    <Column field="name" header="Name" sortable filter filterPlaceholder="Search by name" style={{ minWidth: '14rem' }} body={userBody} />
                    ...
                    <Column field="balance" header="Wallet Coin" sortable dataType="numeric" style={{ minWidth: '12rem' }} body={balanceBody} filter filterElement={balanceFilter} />
